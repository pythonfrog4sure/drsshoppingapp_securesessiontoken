...
export function Shop({ username, onLogout }: ShopProps) {
  ...
  const refreshSecureToken = useCallback(async () => {
    ...
    const t = await getSecureSessionToken('login', 300);
    setSecureToken(t);
    ...
    <div className="shop">
      ...
      <section className="shop-secure-token" aria-label="Secure session token for backend">
        <div className="shop-secure-token-row">
          ...
          {tokenLoading ? (
            ...
            <>
              ...
              <button type="button" className="btn btn-sm" onClick={() => void refreshSecureToken()}>
                Refresh
              </button>
              <button
                type="button"
                className="btn btn-sm btn-primary"
                onClick={() => void navigator.clipboard.writeText(secureToken)}
              >
                Copy
              </button>
            </>
        ...
        <p className="shop-secure-token-hint">
          Send the full value from your app server to Transmit Mosaic APIs (e.g. risk Recommendations) — do not log the raw token in production.
        </p>
      </section>
